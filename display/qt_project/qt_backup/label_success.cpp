...
public:
    DashboardWindow(QWidget *parent = nullptr) : QMainWindow(parent), isFullScreenMode(false) {
        ...
        motorSpeedLabel->setGeometry(200, 400, 300, 100); // 크기 및 위치 설정
        motorSpeedLabel->setStyleSheet("color: white; font-size: 48px; background: transparent;"); // 텍스트 크기 증가
    ...
    void keyPressEvent(QKeyEvent *event) override {
        if (event->key() == Qt::Key_Escape) {
            if (isFullScreenMode) {
                showNormal();
                resize(defaultSize);
                setFixedSize(defaultSize); // 창 크기를 초기 크기로 고정
            } else {
                setFixedSize(QWIDGETSIZE_MAX, QWIDGETSIZE_MAX); // 창 크기 제한 제거
                showFullScreen();
            }
