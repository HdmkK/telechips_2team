...
#define reg_base1 (MCU_BSP_TIMER_BASE + (2UL * 0x100UL)) // 채널에 따라 타이머 베이스 주소 계산
#define reg_main_counter1 (reg_base1 + TMR_MAIN_CNT) // MAIN_CNT 레지스터 주소
...
static sint32 TimerDelayHandler
(
...
*/
//read maincnt
uint32 TIMER_GetCurrentMainCounter1(void) {
    //uint32 reg_base = MCU_BSP_TIMER_BASE + (2UL * 0x100UL); // 채널에 따라 타이머 베이스 주소 계산 2CH
    //uint32 reg_main_counter = reg_base + TMR_MAIN_CNT; // MAIN_CNT 레지스터 주소
    return SAL_ReadReg(reg_main_counter1); // MAIN_CNT 값 읽기
}
uint32 TIMER_GetCurrentMainCounter(void) {
    uint32 reg_base = MCU_BSP_TIMER_BASE + (2UL * 0x100UL); // 채널에 따라 타이머 베이스 주소 계산 2CH
    uint32 reg_main_counter = reg_base + TMR_MAIN_CNT; // MAIN_CNT 레지스터 주소
...
void delay_us2(uint32_t microseconds) {
    uint64_t start_time = 0;
    uint64_t current_time = 0;

    // 시스템 시간 기록
    SAL_GetSystemTickCount(&start_time);

    do {
        SAL_GetSystemTickCount(&current_time);
        mcu_printf("%d\n",current_time);
    } while ((current_time - start_time) < (microseconds / 1000UL)); // ms 단위로 변환
}
...
static sint32 TimerDelayHandler(TIMERChannel_t channel, void *pArgs) {
    ...
    delay_completed = true; // 딜레이 완료 플래그 설정
    (void)GIC_IntSrcDis((uint32)GIC_TIMER_1); // 인터럽트 비활성화
...
)
{
    ...
    {
        mcu_printf("timer doesn't initialized\n");
    ...
    {
        //mcu_printf("timer 동작 시작!\n");
        rate = TIMER_GetRate(pCfg);

        if (rate == 0UL)
        {
            mcu_printf("timer rate error!\n");
            ret = SAL_DbgReportError
            (
                SAL_DRVID_TMR,
                TIMER_API_ENABLE_CFG,
        ...
        else
        {
            ...
            {   
                ...
                {   
                    //mcu_printf("TIMER_Is ready3: Reg Value = 0x%08X\n", reg);
                    ...
                    //mcu_printf("TIMER_Is ready4: Reg Value = 0x%08X\n", reg);
...
)
{
    ...
    else
    {
        ...
        //mcu_printf("ch ret real success\n");
    }
