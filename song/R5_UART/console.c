...
static void CSL_DeviceUart(uint8 ucArgc, void *pArgv[]) {
  ...
  mcu_printf("nice build start\n");

  const char message[] = "\r\n 1. qweplay alive message : [alive] [on/off]\0";
  UART_Write(UART_CH2, (const uint8 *)message, sizeof(message));
  mcu_printf("wait read data\n");
  ...
  while (1) {
    ...
    // 수신된 데이터 처리
    buffer[total_bytes_read] = '\0'; // 문자열 종료
    if (total_bytes_read > 0) {
      mcu_printf("Received Data: %s\n", buffer);
    } else {
      mcu_printf("No data received or failed to read data.\n");
    }
