...
public:
    DashboardWindow(QWidget *parent = nullptr) : QMainWindow(parent), isFullScreenMode(false) {
        ...
        motorSpeedLabel->setGeometry(145, 260, 300, 100); // 초기 크기와 위치 설정
...
protected:
    ...
    void resizeEvent(QResizeEvent *event) override {
        updateScaledImage();
        adjustLabelPositions(); // 창 크기 변경 시 텍스트 위치 조정
...
private:
    ...
    void adjustLabelPositions() {
        for (int i = 0; i < initialSizes.size(); ++i) {
            ...
            if (i == 0) label = motorSpeedLabel;
            else if (i == 1) label = rainingLabel;
            else if (i == 2) label = inTunnelLabel;
            else if (i == 3) label = airConditionLabel;
